"""This module contains class for life game"""
...
class LifeGame:
    """This class implements a version of the classic Game of Life"""
    ...
    def _get_neighbors(self, i: int, j: int) -> Counter[str]:
        ...
        for di, dj in self.directions:
            ...
            if 0 <= ni < self.shape[0] and 0 <= nj < self.shape[1]:
                entity_idx = self.idx_to_entity_idx.get(self.map[ni][nj],
                                                        "new_entity_idx")
    ...
    def _update_map(self) -> None:
        ...
        for i in range(self.shape[0]):
            for j in range(self.shape[1]):
                ...
                if entity_name in ("fish", "shrimp"):
                    neighbors_like_entity_idx = (
                        neighbors[entity_name] +
                        neighbors[f"died_{entity_name}"]
                    )
                    if neighbors_like_entity_idx >= 4 or\
                            neighbors_like_entity_idx <= 1:
                        self.map[i][j] += 1
    ...
    def get_next_generation(self) -> list[list[int]]:
        """
        This function displays the state of the card in the game
        :return: map
        """
        self._update_map()
